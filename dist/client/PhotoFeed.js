...
const PhotoFeed = ({ selectedPreset, presets, addTopicToPreset }) => {
    ...
    const useStaticImages = false; // Cambia esta variable a true para usar imágenes estáticas
    ...
    const fetchPhotos = () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            setPhotos(fallbackImages); // Use fallback images if there's an error
        }
        finally {
            setLoading(false);
    ...
    (0, react_1.useEffect)(() => {
        setPhotos([]); // Clear current photos when preset changes
        fetchPhotos();
    }, [selectedPreset]); // Incluimos selectedPreset para que actualice al cambiar el preset
